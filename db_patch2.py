...
migration_008 = """
    ...
    INSERT INTO driver_beliefs_new (driver, owner_user_pk, fundamental, signal_status, score, confidence, evidence_count, trend, insufficient_reason, scoring_model_version, taxonomy_version, computed_at)
    SELECT driver, 1, fundamental, signal_status, score, confidence, evidence_count, trend, insufficient_reason, scoring_model_version, taxonomy_version, computed_at FROM driver_beliefs;
    ...
    INSERT INTO coach_outputs_new (output_pk, driver, car, track, owner_user_pk, payload_version, prompt_version, model, output_json, created_at)
    SELECT output_pk, driver, car, track, 1, payload_version, prompt_version, model, output_json, created_at FROM coach_outputs;
    ...
    CREATE TABLE garage61_sync_state_new (
        ...
        track TEXT NOT NULL,
        owner_user_pk INTEGER NOT NULL,
        laps_seen INTEGER NOT NULL,
        laps_new INTEGER NOT NULL,
    ...
    INSERT INTO config_history_new (change_pk, owner_user_pk, key, old_value, new_value, source, note, applied_at)
    SELECT change_pk, 1, key, old_value, new_value, source, note, applied_at FROM config_history;
...
replacements = [
    ...
    (
        '''        rows = self.conn.execute(
            "SELECT change_pk, key, old_value, new_value, source, note, applied_at FROM config_history ORDER BY change_pk"
        ).fetchall()''',
        '''        rows = self.conn.execute(
            "SELECT change_pk, key, old_value, new_value, source, note, applied_at FROM config_history WHERE owner_user_pk = ? ORDER BY change_pk",
            (self.user_pk,)
        ).fetchall()'''
    ),
    (
