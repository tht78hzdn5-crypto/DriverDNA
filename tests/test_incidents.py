...
def test_payload_has_incidents_section_coach_and_chat_grounded(tmp_db):
    """Incidents are deterministic engine output and belong in the full
    payload (report + UI); the coach payload carries them too, grounded via
    Layer 3 (SPEC.md A20) — each event's coaching_principle_id is the
    engine's own deterministic verdict, citable by the AI. Chat's live Q&A
    path gained the same grounding in Track B3 (docs/UI-V3-PLAN.md,
    chat-v3): incidents ride in the bundle like every other section."""
    ...
    assert "incidents" in build_chat_bundle(tmp_db, **cohort)["report"]
