...
- **U4 — Packaging & polish.** `driverdna ui` command, built assets shipped in-package, offline verification, design pass against this document, and the static HTML report templates migrated onto `ui/tokens.json` so both surfaces share one appearance (report determinism tests must stay green through the restyle).
