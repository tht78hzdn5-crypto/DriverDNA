...
Do not begin a milestone until the prior one's done-criteria (in the spec)
pass. Every milestone ends with tests green AND its inspectable artifact
generated from the real fixtures and reviewed.

Both milestone tracks are complete — see `CLAUDE.md`'s "Current status" for
...
Multiple agents work on this repository, one at a time. The rules below keep
a handoff cheap in either direction, so no agent has to guess what the last
one did.
...
`tests.yml` runs the full suite on every push and PR to `main`, on Python 3.11
and 3.12, with a Postgres service container so the dual-backend tests execute.
A follow-up step fails the job if any test skipped for "no local Postgres
configured" — otherwise a container that never came up would leave every A23
guard skipped with the job still green.

It does **not** install Chromium, so the two browser trust-gate tests
(`test_render_parity.py`, `test_offline.py`) skip in CI — the owner's to run
locally. **Green CI is not proof the browser trust gates hold.** Say so
